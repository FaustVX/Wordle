...
{
    private static async Task RunWordle(
    ...
    {
        for (var (game, customize) = (new Game(wordLength, tries, isRandom, WordList.WordLists[language]), false); true; (game, customize) = (customize ? StartWordle(game) : game.Recreate(), false))
        {
            Console.Clear();
            WriteHeader(game);
            Console.WriteLine();
    ...
    private static void PrintScores(BaseGame game, bool found)
    {
    ...
    private static async Task<IEnumerable<string>> GetDefinition(string word)
    {
    ...
    private static Game StartWordle(Game previous)
    {
    ...
    private static string? Input(Game game)
    {
